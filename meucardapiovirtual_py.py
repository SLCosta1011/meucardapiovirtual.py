...
# ===============================================================
# 📘 ASSISTENTE DE CARDÁPIO - IA PDF (PIZZARIA)
# ===============================================================
...
uploaded = files.upload()  # Selecione seu arquivo .pdf (cardápio)
if not uploaded:
...
print("📂 Caminho completo:", PDF_PATH)
...
# Configurações de Chunking (divisão do PDF em pedaços)
CHUNK_SIZE = 1800
CHUNK_OVERLAP = 250
...
def diagnostico_pdf(blocos: List[str], preview_chars: int = 400):
    print(f"📄 {len(blocos)} blocos carregados.")
    if blocos:
        print(f"🧩 Primeiro bloco ({len(blocos[0])} chars):\n", blocos[0][:preview_chars])
    else:
        print("⚠️ Nenhum texto extraído (pode ser PDF escaneado).")

# ===============================================================
# 🔍 Seleção de blocos relevantes
# ===============================================================
...
def perguntar_openrouter(pergunta: str, contexto: str, model: str = MODEL_NAME) -> str:
    if not OPENROUTER_API_KEY or not OPENROUTER_API_KEY.startswith("sk-"):
        return "⚠️ API Key inválida. Configure sua chave do OpenRouter."

    system_msg = (
        "Você é um assistente de cardápio. "
        "Responda com base nas informações do PDF (sabores, ingredientes e preços). "
        "Responda o preço do pedido, somando o valor da pizza com o valor da bebida, caso o usuário peça as duas coisas. Se não, responda sobre o pedido, inclusive se forem separados"
        "As formas de pagamento são: pix, débito e cartão de crédito (com parcela a vista(em uma vez)). Não mencione isso até o usuário perguntar"
        "APENAS após o usuário perguntar sobre o pagamento e digitar a forma de pagamento diga: Obrigado pela preferência, tenha um bom apetite"
        "Se não conseguir encontrar a informação, diga exatamente: 'Desculpe, não tenho acesso a essa informação.'"
    )
...
Responda com base no cardápio.
""".strip()

    headers = {
        "Authorization": f"Bearer {OPENROUTER_API_KEY}",
        "Content-Type": "application/json",
        "X-Title": "Cardápio IA - Pizzaria",
    }
...
def responder_pergunta(pergunta: str):
    """Processa a pergunta do usuário, busca no PDF e responde usando o modelo."""
    print("🤔 Só um momento...")
